...
class ReadFromAudioDiscJob : ReadFromDiscJob
{
    ...
    SectorRange sectorRange( Disc disc )
    {
        ...
        {
            // Out of bounds?
            if ( _fromTrack > tracks.length || _toTrack > tracks.length )
            {
                return failure;
            }

            // Is audio track?
            if ( !tracks[ _fromTrack - 1 ].isAudio() )
            {
                return failure;
            }
            // Is audio track?
            if ( !tracks[ _toTrack - 1 ].isAudio() )
            {
                return failure;
            }
